...
def vp_start_gui():
    ...
    global val, w, root
    root = Tk()
    top = Creat_Square (root)
    root.mainloop()

w = None
def create_Creat_Square(root, data_obj, canvas, shape=None, *args, **kwargs):
    '''Starting point when module is imported by another program.'''
    global w, w_win, rt
    rt = root
    w = Toplevel (root)
    top = Creat_Square (data_obj, canvas, shape, w)
    return (w, top)

def destroy_Creat_Square():
    global w
    w.destroy()
    w = None
...
class Creat_Square:
    def __init__(self, data_obj, canvas, shape, top=None):
        ...
        if shape is None:
            top.title("Creat Square")
        else:
            top.title("Mode Square")
        ...
        self.txt_Square = Label(top)
        self.txt_Square.place(relx=0.062, rely=0.044, height=21
                , width=129)
        self.txt_Square.configure(background="#d9d9d9")
        self.txt_Square.configure(disabledforeground="#a3a3a3")
        self.txt_Square.configure(foreground="#000000")
        self.txt_Square.configure(text='''Creat Square''')
    ...
    def button_pres(self):
        ...
        if x.isdigit() and y.isdigit() and side.isdigit() and color is not "" and color in COLORS and line in COLORS and int(x) < 1001 and int(y) < 916:
            ...
            obj = [Square(int(x), int(y), int(side), color, line),[int(x), int(y), int(side), color, line]]
